...
│       ├── probe.py                       # Reads a model's /describe endpoint
│       ├── predict_client.py              # Generic predict_fn, self-correcting on schema errors
│       ├── synthetic_data.py              # LLM-generated + validated synthetic data, top-up retries
│       ├── importance.py                  # LIME loop across synthetic points, ranks + normalizes
│       ├── pdp_step.py                    # PDP on top 4 numeric + top 4 categorical features
│       ├── case_examples.py               # 2 individual sample-case walkthroughs (reuses LIME results)
│       ├── plotting.py                    # All deterministic chart generation
│       ├── report_generator.py            # LLM report writing + safe marker-based chart embedding
│       ├── pdf_export.py                  # Markdown -> PDF (pandoc + wkhtmltopdf)
│       └── run_report.py                  # Orchestrates the full pipeline end to end
│
├── requirements.txt
└── README.md
```
...
- `POST /{dataset}/{model_key}/predict` - takes raw feature values (not preprocessed), returns a prediction in original units (`predicted_price` for regression, `predicted_class` + `predicted_probability` for classification).
- `GET /{dataset}/{model_key}/describe` - returns job type, per-feature schema (numeric stats or categorical categories/frequencies, target excluded), an example request body, and which output field to read.
...
Output lands under `./explainability_reports/<sanitized_base_url>__<model_key>/` in whichever directory the script is run from.

### Running it

```bash
export OPENAI_API_KEY=sk-...
cd demo_serving && ./run_server.sh                    # separate terminal
cd src/pipeline
python run_report.py http://localhost:8000/house_prices linear
```
